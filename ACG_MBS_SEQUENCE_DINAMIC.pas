...
function ComparaAdecuacionCromoSeq(Item1, Item2: Pointer): Integer;
begin
...
begin
  ...
  begin
    if j<i then a[j]:=seq[j];
    if j=i then a[j]:=instruccionAVRaleatoria;
    if j>i then a[j]:=seq[j-1];
  end;
  SetLength(seq,rds.s+1);
  seq:=Copy(a);
...
var
  nalea:Integer;
begin
  reemplazarInstruccionAVR(seq[i]);
  rds.p:=5;
end;
...
begin
  SetLength(seq,rds.s+1);
  rds.s:=rds.s+1;
  seq[rds.s-1]:=instruccionAVRaleatoria;
  rds.p:=1;
end;
...
begin
  ...
  SetLength(seq,rds.s);
  rds.p:=1;{Procedencia Multiobjetivo}
...
//  PopList.Sort(@ComparaAdecuacionCromoSeq);
  PopList.Sort(@ComparaAdecuacionCromoSeqConSumafesc);
...
(*
  ...
  begin
    ...
    7..9:begin
      n1:=x1.e+random(x1.s-x1.e);
      n2:=x2.e+random(x2.s-x2.e);
    end;
    end;
  end;
*)
  ...
  r1[n1]:= r2[n2];
  r2[n2]:= vi;
  Qt.PopList.add(TCromSeqDin.createWithSeqProcede(r1,8));
  Qt.PopList.add(TCromSeqDin.createWithSeqProcede(r2,8));
end;
...
begin
  ...
  Qt.PopList.add(TCromSeqDin.createWithSeqProcede(r1,9));
  Qt.PopList.add(TCromSeqDin.createWithSeqProcede(r2,9));
end;
